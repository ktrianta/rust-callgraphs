...
impl<'a, 'tcx> TableFiller<'a, 'tcx> {
    ...
    pub fn resolve_def_id(&mut self, def_id: hir::def_id::DefId) -> types::DefPath {
        ...
        self.tables.register_def_paths(
        ...
        if self.span_registry.contains_key(&span) {
            ...
            if let ExpnKind::Macro(_, symbol) = expansion_data.kind {
                self.tables.register_macro_expansions(
                    interned_span,
                    symbol.to_string(),
                    source_map.span_to_string(expansion_data.def_site),
                );
            }
            ...
            *interned_type
            ...
            match typ.kind {
                ...
                ty::TyKind::Adt(adt_def, substs) => {
                    ...
                    let variant_def_path = self.resolve_def_id(adt_def.did);
                    for (i, variant) in adt_def.variants.iter_enumerated() {
                        ...
                        for field in &variant.fields {
                            ...
                            self.tables.register_types_adt_field(
                                interned_type,
                                variant_index,
                                field_def_path,
                    ...
                    for predicate in binder.skip_binder().iter() {
                        match predicate {
                            ty::ExistentialPredicate::Trait(trait_ref) => {
                                ...
                                self.tables
                                    .register_types_dynamic_trait(interned_type, def_path);
                            }
                            ...
                            ty::ExistentialPredicate::AutoTrait(def_id) => {
                                ...
                                self.tables
                                    .register_types_dynamic_auto_trait(interned_type, def_path);
                            }
                        }
                    }
                    interned_type
